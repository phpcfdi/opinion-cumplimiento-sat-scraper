...
final class SamlFormHandlerTest extends TestCase
{
    public function testHandleSamlRedirectProcessesFormSuccessfully(): void
    {
        ...
        $client->expects($this->exactly(2))
            ->method('request')
            ->willReturnCallback(function (string $method, string $url, array $options = []) use ($redirectUrl, $samlFormHtml, $samlAction) {
                if ('GET' === $method && $url === $redirectUrl) {
                    $this->assertArrayHasKey('headers', $options);
                    return new Response(200, [], $samlFormHtml);
                }
                if ('POST' === $method && $url === $samlAction) {
                    $this->assertArrayHasKey('form_params', $options);
                    $this->assertIsArray($options['form_params']);
    ...
    public function testHandleSamlRedirectWithMultipleHiddenFields(): void
    {
        ...
        $client->expects($this->exactly(2))
            ->method('request')
            ->willReturnCallback(function (string $method, string $url, array $options = []) use ($samlHtml) {
                if ('GET' === $method) {
    ...
    public function testHandleSamlRedirectSendsCorrectHeaders(): void
    {
        ...
        $client->expects($this->exactly(2))
            ->method('request')
            ->willReturnCallback(function (string $method, string $url, array $options = []) use ($referer, $samlHtml, $redirectUrl) {
                if ('GET' === $method) {
                    $this->assertArrayHasKey('headers', $options);
    ...
    public function testHandleSamlThrowsExceptionWhenNotFormIsFound(): void
    {
        $redirectUrl = 'https://ptsc32d.clouda.sat.gob.mx/callback';
        $referer = 'https://login.mat.sat.gob.mx/auth';
        ...
        $samlHtml = <<<HTML
            ...
            <html>
            <body>
                <p>It does not contain any form</p>
            </body>
            </html>
            HTML;
        ...
        $client->expects($this->exactly(1))
            ->method('request')
            ->willReturnCallback(fn () => new Response(200, [], $samlHtml));

        $handler = new SamlFormHandler($client);

        $this->expectException(SatException::class);
        $this->expectExceptionMessage('Not found SAML form in the response');
